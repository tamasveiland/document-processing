#!/usr/bin/env python3
"""
Process a PDF with Azure Document Intelligence (prebuilt-layout) — parallel figures.

Identical to process_document_di.py except that figure description saves and
image downloads are performed concurrently via a ThreadPoolExecutor.

Usage
-----
    python src/process_document_di_parallel.py path/to/document.pdf
    python src/process_document_di_parallel.py path/to/document.pdf --output-dir results/
    python src/process_document_di_parallel.py path/to/document.pdf --model-id prebuilt-read
    python src/process_document_di_parallel.py path/to/document.pdf --max-workers 16
...
# ---------------------------------------------------------------------------
# Figure helpers (thread-safe per-figure work)
# ---------------------------------------------------------------------------
...
def _process_single_figure(
    ...
    """Process one figure (description + image download).

    Returns (figure_id, elapsed_seconds, log_messages).
    All I/O is self-contained so this is safe to call from a thread.
    """
    t0 = time.perf_counter()
    messages: list[str] = []
...
# ---------------------------------------------------------------------------
# Parallel figure saving
# ---------------------------------------------------------------------------


def _save_figures_parallel(
    client: DocumentIntelligenceClient,
    ...
    """Save all figures concurrently using a thread pool."""
    ...
    workers = min(max_workers, figure_count)
    print(f"  Processing {figure_count} figure(s) with {workers} worker(s) ...")
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            _fig_id, _elapsed, messages = future.result()
            for msg in messages:
                print(msg)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Process a PDF with Azure Document Intelligence (parallel figures).",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Max parallel threads for figure downloads (default: 8).",
    )
    ...
    if args.save_extras:
        ...
        t = time.perf_counter()
        _save_figures_parallel(
            client, result, args.model_id, result_id, output_dir,
